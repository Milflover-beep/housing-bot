...
- **Options**:
  ...
  - `rank-type` (choice, required) — `Prime` | `Elite` | `Apex` | `PM` (must match the ladder you’re checking; **PM** has special rules below).
- **Behavior**:
  - **PM**: Does **not** call Hypixel (no network level requirement), does **not** read `tier_results` ladder rules, and **application denial cooldown does not apply** (they can pass while on cooldown). Only **blacklists** and **admin blacklists** can make the check **not eligible** (red). The embed’s Hypixel field shows that level is not required. Alts follow-up and applicant role behavior are unchanged.
  - **Prime / Elite / Apex**: Calls the **Hypixel** `GET /v2/player` API (`HYPIXEL_API_KEY`) using `player.networkExp` and the standard formula for **network level**. When the API **succeeds**, players must be **network level 30+** with a real profile or the check is **not eligible** (red). When the API **fails** (HTTP errors, rate limits, missing key, bad JSON, etc.), **Hypixel is not enforced** for that run — pass/fail follows only the other rules below, and an embed field tells staff to use **`/hypixel`** to verify level manually. Then queries `blacklists` (active: no expiry or expiry in the future), `admin_blacklists` (non‑pardoned), alts, latest `tier_results` per ladder, and **application denial cooldown** (`application_denials` where `cooldown_until > NOW()` for that Discord user). **Blacklist / admin blacklist / active denial cooldown** set **not eligible** (red); cooldown shows **when it ends** (Discord timestamps). **Apex** tryouts **fail** unless there is **Elite** or **Apex** on file (Prime‑only or **no tier** cannot apply for Apex; existing Apex counts for re‑apply). **Prime** and **Elite** have no “must hold a lower ladder” gate in `/check`. Having a **higher** ladder on file does **not** block or warn when checking a **lower** ladder (e.g. Elite on file can pass `/check` for Prime). **Known alts** are not included in the public embed; the invoker gets an **ephemeral** follow-up with alt details (PM+ and Staff+). **PM-only** runners get an extra line instructing them to **ping a Manager** and include that information. Full pass (green) assigns the applicant role when configured (including when the only extra context was alts).
...
- **Description**: List active punishments with DB IDs (quick lookup for moderation).
- **Default permission**: Manage Roles (**Staff+**).
- **Behavior**: Shows active `punishment_logs` rows (`status=active`, `punishment_status=active`) with `#id`, player, short details, and remaining time/permanent marker.
